...
export default function Home() {
    ...
    try {
      ...
      const jobForDb: JobData = {
        ...
        finishedDate: jobData.finishedDate
          ? jobData.finishedDate.toISOString()
          : undefined,
        completedDate: jobData.completedDate
          ? jobData.completedDate.toISOString()
          : undefined,
  ...
  const handleUpdateJob = async (
    ...
    try {
      ...
      if (updatedJobData.customer !== undefined)
        jobForDb.customer = updatedJobData.customer;
      if (updatedJobData.jobName !== undefined)
        jobForDb.jobName = updatedJobData.jobName;
      if (updatedJobData.jobNumber !== undefined)
        jobForDb.jobNumber = updatedJobData.jobNumber;
      if (updatedJobData.projectManager !== undefined) {
        // Only add projectManager if it's not empty
        if (
          updatedJobData.projectManager &&
          updatedJobData.projectManager.trim() !== ""
        ) {
      ...
      if (updatedJobData.startDate !== undefined) {
        jobForDb.startDate =
          updatedJobData.startDate instanceof Date
            ? updatedJobData.startDate.toISOString()
            : updatedJobData.startDate === null
              ? undefined
              : (updatedJobData.startDate as string);
      }
      if (updatedJobData.finishedDate !== undefined) {
        jobForDb.finishedDate =
          updatedJobData.finishedDate instanceof Date
            ? updatedJobData.finishedDate.toISOString()
            : updatedJobData.finishedDate === null
              ? undefined
              : (updatedJobData.finishedDate as string);
      }
      if (updatedJobData.completedDate !== undefined) {
        jobForDb.completedDate =
          updatedJobData.completedDate instanceof Date
            ? updatedJobData.completedDate.toISOString()
            : updatedJobData.completedDate === null
              ? undefined
              : (updatedJobData.completedDate as string);
      }
      if (updatedJobData.priority !== undefined)
        jobForDb.priority = updatedJobData.priority;
  ...
  const handleUpdateTask = async (
    ...
    try {
      ...
      if (updatedTaskData.title !== undefined)
        taskForDb.title = updatedTaskData.title;
      if (updatedTaskData.description !== undefined)
        taskForDb.description = updatedTaskData.description;
        ...
        taskForDb.dueDate =
          updatedTaskData.dueDate instanceof Date
            ...
            : updatedTaskData.dueDate === null
              ? undefined
              : (updatedTaskData.dueDate as string);
      }
      if (updatedTaskData.priority !== undefined)
        taskForDb.priority = updatedTaskData.priority;
      if (updatedTaskData.status !== undefined)
        taskForDb.status = updatedTaskData.status;
      if (updatedTaskData.assignedTo !== undefined)
        taskForDb.assignedTo = updatedTaskData.assignedTo;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {activeTab === "stats" && <StatsPage jobs={jobs} />}
    </div>
  );
}
