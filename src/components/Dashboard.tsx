...
export const Dashboard: React.FC<DashboardProps> = ({ jobs, onAddJob }) => {
  ...
  const highPriorityJobs = currentJobs.filter(job => job.priority === 'High').length;
  const inProgressJobs = currentJobs.filter(job => job.startDate && !job.completedDate).length;
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {currentJobs.slice(0, 8).map((job) => (
                      <tr key={job._id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                            job.completedDate ? 'bg-green-100 text-green-800' :
                            job.startDate ? 'bg-blue-100 text-blue-800' :
                            'bg-gray-100 text-gray-800'
                          }`}>
                            {job.completedDate ? 'Completed' : job.startDate ? 'In Progress' : 'Not Started'}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <button
