...
/**
 * Sort jobs by status: Pending -> In Progress -> Waiting Submittal -> Completed
...
export const sortJobsByStatus = (jobs: IJob[]): IJob[] => {
  return [...jobs].sort((a, b) => {
    const getStatusPriority = (job: IJob) => {
      if (job.completedDate && job.completedDate !== null) return 4; // Completed - lowest priority
      if (job.waitingSubmittalDate && job.waitingSubmittalDate !== null) return 3; // Waiting Submittal
